...
namespace sc {
  ...
  void ScreenCaptureDuplicateOutputDirect3D11::updateV2(uint8_t** ppData, int* psize, bool* result) {
      ...
      if (true == has_frame) {
          ...
          if (S_OK != hr) { printf("Error: failed to release the frame right before acquiring the next one.\n"); }
      }

      *result = false;
      /* Acquire a new frame, and directly return (0) when there is no new one. */
      hr = duplication->AcquireNextFrame(150, &frame_info, &frame);
      ...
      if (S_OK == hr) {
          ...
          if (S_OK == hr) {
              ...
              renderer.scaleV2(frame_tex, ppData, psize);
              *result = true;
          }
          // 第一帧总是0
