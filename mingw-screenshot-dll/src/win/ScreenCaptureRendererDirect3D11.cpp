...
static  void *gpframe_data = NULL; //全局桌面数据指针，需要自己malloc 分配内存
...
namespace sc {
  ...
  /*
    @todo we're setting some state which may be cached and therefore
     ...
     work.
  */
 int ScreenCaptureRendererDirect3D11::scaleV2(ID3D11Texture2D* tex,uint8_t **ppData, int *psize) {
   if (NULL == tex) {
    ...
    if (S_OK == hr) {
      ...
      // printf("NBYTES: %u\n", map.DepthPitch);
      if(gpframe_data == NULL){
        gpframe_data = malloc(map.RowPitch*settings.output_height);
      }
      ...
      for (int h = 0; h < settings.output_height; h++) {
          for (int w = 0; w < settings.output_width; w++) {
              ...
              char* r = &((char*) gpframe_data)[h * settings.output_width * 4 + w * 4 + 2];
              // BGRA==>RGBA
              char temp;
              temp = *b;
              *b   = *r;
...
#endif      
      *ppData = (uint8_t*)gpframe_data;
      *psize  = map.RowPitch * settings.output_height;
